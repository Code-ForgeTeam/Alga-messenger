...
const RECENT_EMOJI_STORAGE_KEY = 'moon:recent-emojis';
...
export default function ChatPage() {
  ...
  const downloadAttachmentLegacyA = async (attachment: Attachment) => {
    ...
    try {
      ...
      const path = `${mediaFolder}/Moon/${nativeFileName}`;
  ...
  const downloadAttachmentLegacyB = async (attachment: Attachment) => {
    ...
    const saveOnNative = async (): Promise<boolean> => {
      ...
      const folder = attachment.type === 'video' ? 'Moon/Videos' : 'Moon/Photos';
  ...
  const downloadAttachment = async (attachment: Attachment) => {
    ...
    if (!url) return;
    {
      ...
      const localPath = `moon-downloads/${Date.now()}-${sanitizeFileName(fileName)}`;
        const base64 = await blobToBase64(blob);
        await Filesystem.writeFile({
          path: localPath,
          data: base64,
  ...
  const downloadAttachmentStable = async (attachment: Attachment) => {
    ...
    const saveInNativeGallery = async (): Promise<boolean> => {
      ...
      const filePath = `moon-downloads/${Date.now()}-${fileName}`;
